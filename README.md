...
`start.ps1` 是前台启动模式，会在当前终端持续显示后端请求日志和前端开发服务器日志。Windows 资源管理器中双击 [start.cmd](start.cmd) 即可打开同一个前台模式；`.ps1` 文件被双击时可能按系统文件关联打开记事本，这是 Windows 的默认行为。

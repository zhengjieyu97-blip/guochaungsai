...
async function load() {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : '读取对象失败'
  } finally {
    loading.value = false
  }
}
...
async function checkIn(subject: SubjectItem) {
  try {
    const updated = await api.checkIn(subject.id)
    Object.assign(subject, updated)
    push(`${subject.name} 的平安签到已记录`)
  } catch (err) {
  ...
  <div class="page-wrap">
    ...
    <section class="panel">
      <div class="panel-head">
        <div>
          <h2>对象花名册</h2>
          <p>查看健康风险标签与最近平安签到记录</p>
        </div>
        <div class="filters" style="padding: 0; background: transparent; border: 0;">
          <label class="search-field" style="min-width: 240px;">
            <Search :size="16" />
            <input v-model="filters.search" placeholder="搜索姓名或编号" />
          </label>
      ...
      <div v-if="loading" class="loading-state">正在读取照护对象…</div>
      <div v-else-if="error" class="empty-state">
        <HeartPulse :size="32" />
        <strong>对象档案暂时读不到</strong>
        <p>{{ error }}</p>
      ...
      <div v-else class="people-grid" style="padding: 20px;">
        ...
        >
          <span class="person-avatar" :class="{ child: person.subject_type === 'CHILD' }">
            {{ person.name.slice(0, 1) }}
          </span>

          <span class="person-main">
            <strong>{{ person.name }}</strong>
            <span class="person-subline">
              <span class="type-chip" :class="person.subject_type.toLowerCase()">
                {{ person.subject_type_label }}
              </span>
              {{ person.age }} 岁 · {{ person.building_text }}
            </span>
            <span class="tag-list">
              <span v-for="tag in person.risk_tags" :key="tag" class="risk-tag">{{ tag }}</span>
              <span v-if="!person.risk_tags.length" class="risk-tag empty">暂无风险标签</span>
            </span>
          </span>

          <span class="person-meta">
            <span class="open-count" :class="{ zero: person.open_event_count === 0 }">
              {{ person.open_event_count }}
            </span>
            <span class="open-label">未闭环事件</span>
            <span class="icon-button" style="margin-top: 4px;">
    ...
    <p v-if="canCheckIn" class="page-footnote">
      <Check :size="16" />
      可在对象详情页记录平安签到；签到会同步刷新相关开放事件的风险评估原因。
    </p>
  </div>
</template>
